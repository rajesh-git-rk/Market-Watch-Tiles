...
public class ScriptImp2 {
 ...
 public List<Alerts> getTrigger(Alerts script) {
	...
	//			+ "AND expirydate='"+script.getExpirydate().trim()+"'AND timer='"+script.getTimer().trim()+"' AND status='1'");
		...
		qry=session.createQuery("from Alerts where  price <= '"+script.getPrice()+"' AND status='1' AND userId='"+script.getUserId()+"'");
   ...
   public List<MarketWatch> reNameMarketWatchList(MarketWatch script) {
	    ...
	    public List<MarketWacthScript> scripListOfMarketId(MarketWacthScript script) {
           ...
           int marketwatchId=script.getMarketwatchid().getMarketwatchid();
	    	...
	    	list = session.createQuery("from MarketWacthScript where marketwatchid="+marketwatchId+" and status='1' order by Id desc").list();
		...
		public List<SettlementDownload> getsettleId(String datestring){
			List<SettlementDownload> list=null;
			try {
			
				Session session = this.sessionFactory.getCurrentSession();
		    	
		    	list = session.createQuery("from SettlementDownload where tradestartdate='"+datestring+"'").list();
...
public int createMarketWatch(String userid) {
	...
	for(int i=1;i<=5;i++) {
	 ...
	 String marketvalue= String.valueOf(i);
	 
	 mar.setMarketwatch_name(marketvalue);
	 mar.setUserId(userid);

	 
     marketval=(Integer) session.save(mar);
     
    // System.out.println("marketval::::"+marketval);
...
public List<MarketWacthScript> searchToken(String id) {
	...
	list = session.createQuery("from MarketWacthScript where token='"+id+"'").list();		
...
public List<MarketWatch> getMarketWatchlist() {

	List<MarketWatch> list=null;
	try {
	Session session = this.sessionFactory.getCurrentSession();    	
	
	list = session.createQuery("from MarketWatch").list();	
...
public int orderNotificationInsert(OrderNotification ordernotification) {
	
	Session session = this.sessionFactory.getCurrentSession();

	int val=(Integer) session.save(ordernotification);
	
	return val;
}

public List<OrderNotification> getNotificationOrder(OrderNotification script) {
	
	List<OrderNotification> list=null;
	
	boolean userFound = false;
Session session = this.sessionFactory.getCurrentSession();
Query qry=null;

qry=session.createQuery("from OrderNotification where  notificationdate = '"+script.getNotificationdate()+"'  AND userId='"+script.getUserId()+"' ORDER BY notificationtime desc");
