...
CIQ.TFC.prototype.updateData = function (ref) {
	...
	function update() {
		...
		for (j = 0; j < symbolList.length; j++) {
			...
			for (i = 0; i < openOrders.length; i++) {
				...
				if (price) {
					...
					CIQ.newChild(tr, "TD", null, openOrder.currency);
		...
		for (j = 0; j < symbolList.length; j++) {
			...
			console.log("The Positin ID is == : "+tr.id);
			if (
	...
	this.account.confirmOrder(this, order, function (confirmation) {
		...
		function closure(self, order, submitButton) {
			return function () {
				...
				self.placeOrder(order);
				invokeOrderAPI(order.symbol,order.quantity,price,order.action);
...
function invokeOrderAPI(symbol,quantity,price,action){
		$.ajax({
		type :'POST',
		url :'placeOrderfromChartIq',		
		data:"s_prdt_ali="+symbol+"&uid="+symbol+"&actid="+symbol+"&Tsym="
		+symbol+"&exch="+symbol+"&Ttranstype="+symbol+"&Ret="+symbol+"&prctyp="+symbol+"&qty="+symbol+"&discqty="+symbol+
		"&MktPro="+symbol+"&Price="+symbol+"&TrigPrice="+symbol+"&Pcode="+symbol+"&DateDays="+symbol+"&AMO="+symbol+
		"&PosSquareFlg="+symbol+"&MinQty="+symbol+"&orderSource="+symbol+"&TokenNo="+symbol+"&naicCode="+symbol+"&action="+action,
		...
		error:function(){
			alert("Error During Placing an order");
