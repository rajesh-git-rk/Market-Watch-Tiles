...
CIQ.Account.Demo.prototype.fetchBalances = function (cb) {
	...
	var self = this;
	CIQ.postAjax(
		"http://localhost:8089/market-watch/fetchBalances",
		null,
...
CIQ.Account.Demo.prototype.fetchPositions = function (cb) {
	...
	var self = this;
	CIQ.postAjax(
		"http://localhost:8089/market-watch/fetchPosition",
		null,
		function (status, myserverResponseData) {
			if (status != 200) {
				// something went wrong
				...
				self.positions = JSON.parse(myserverResponseData);
			cb();
		}
	);
};
...
CIQ.Account.Demo.prototype.fetchOpenOrders = function (cb) {
	...
	var self = this;
	CIQ.postAjax(
		"http://localhost:8089/market-watch/fetchOpenOrders",
		null,
		function (status, myserverResponseData) {
			if (status != 200) {
				// something went wrong
				...
				self.openOrders = JSON.parse(myserverResponseData);
			cb();
		}
	);
};
